...
public partial class GeoXmlDepositsByPages : System.Web.UI.Page
{
	...
	protected void Page_Load(object sender, EventArgs e) {
        string pagesString = Request.QueryString["pageId"]; // integers separated with ;
        ...
        foreach (Deposit_GetByPageIdsResult d in deposits)
        {
            if (d.BK_East != null && d.BK_North != null)
            {
              ...
              var wgsPos = transformRT90Coords(Convert.ToDouble(d.BK_North), Convert.ToDouble(d.BK_East));
                ...
                xmlResponse.AppendLine("<link></link>");
                ...
                xmlResponse.AppendLine("<georss:point>" + wgsPos.Latitude.ToString(CultureInfo.GetCultureInfo("en-US")) + ", " + wgsPos.Longitude.ToString(CultureInfo.GetCultureInfo("en-US")) + "</georss:point>");
                xmlResponse.AppendLine("</item>");
            }
            else if (d.SGU_East != null && d.SGU_North != null)
            {
              var wgsPos = transformRT90Coords(Convert.ToDouble(d.SGU_North), Convert.ToDouble(d.SGU_East));
        ...
        //XElement elm = new XElement("deposits");

        //foreach (Deposit d in deposits)
        //{
        //    elm.Add(new XElement("deposit",
        //        new XAttribute("East", d.RT90_East),
        //        new XAttribute("North", d.RT90_North)));
        //}
