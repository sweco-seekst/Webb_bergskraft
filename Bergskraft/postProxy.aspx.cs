        ...
        if (Request.QueryString.AllKeys.Length > 1)
        {
            foreach (string s in Request.QueryString.AllKeys)
            {
                if (s != null)
                {
                    if (s == "url")
                    {
                        remoteUrl = Request.QueryString[s].ToString();
                    ...
                    {
                        remoteUrl = remoteUrl + s + "=" + Request.QueryString[s].ToString() + "&";
                    }
                }

            }
            //Remove last &
            remoteUrl = remoteUrl.Remove(remoteUrl.Length - 1, 1);
        }
        else
        {
            remoteUrl = Request.QueryString["url"].ToString();
        }
